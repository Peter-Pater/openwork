...
// Session read-model helpers used by the inline spatial polling/endpoints.
// (The session route refactor made listWorkspaceSessions a private closure, so
// we keep a small inline copy below for the token-less spatial endpoints.)
import { buildSessionList, buildSessionStatuses } from "./session-read-model.js";
import {
...
// Lists sessions for the token-less spatial (XR) endpoints. The upstream route
// refactor turned the original module-level helper into a private closure
// inside registerSessionRoutes, so we keep this minimal copy for our inline use.
async function listWorkspaceSessions(
  ...
  try {
    ...
    // Spatial endpoints are best-effort; callers handle an undefined result.
    return undefined;
  }
}
...
function createRoutes(
  ...
  // --- OpenWork-XR additions (not part of upstream's route modules) ---
  // Spatial (XR) client endpoints + token-less Google Workspace file listing.
  // These remain registered inline here. (Future: extract to routes/spatial.ts.)
